...
@section('title', 'Target Wise Sales')
@section('header-css')
...
@section('content')
    ...
    <div class="alert alert-success alert-dismissible">
      <button type="button" class="close" data-dismiss="alert">&times;</button>
        ...
        <form role="form" id="item_history_form" method="get" action="{{url('doctor/sales/target-wise')}}">
                    ...
                    <div style="margin-top: 6px;"> <!--Start doctor div-->
                      @foreach($customer->doctors as $doctor)
                       <h2 class="bg-info p-2">{{$doctor['name']}}</h2>
                         ...
                         <!--all point overall end-->

                       <table class="table table-bordered">
                             <thead>
                              <tr><th colspan="5"><center>{{$doctor['name']}}</center></th></tr>
                                <tr>
                                   <th>Sr No.</th>
                                   <th>Points</th>
                                   <th>Total Qty</th>
                             ...
                             <tbody id="{{'all_points_'.$doctor['id']}}">
                          <?php   $j=1; ?>  

                              @foreach($doctor['points'] as $point)
                             <tr>
                                    
                                    <td>{{$j}}</td> 
                                    <td>{{$point['name']}}</td> 
                                    <td id="{{'p_q_'.$point['id']}}"></td> 
                                  
                                      <td id="{{'p_v_'.$point['id']}}"></td>
                       ...
                       @foreach($doctor['points'] as $point) <!--start point-->
                          ...
                          <table class="table table-bordered">
                             <thead>
                              <tr><th colspan="5"><center>{{$point['name']}}</center></th></tr>
                                <tr>
                                   <th>Sr No.</th>
                                   <th>Item</th>
                                   <th>Qty</th>
                             ...
                             <tbody>
                              <?php $ts=$point->saled_items(); $i=1; $qty=0; $amount=0; ?>
                             ...
                             <tr>
                                  ...
                                  <?php  $amount+=$item['amount']; $qty+=$item['qty']; $i++; ?>  
                                  @endforeach
                              ...
                              var id="{{$point['id']}}"; var qty="{{$qty}}"; var val="{{$amount}}";
                                    //alert(qty);
                                      
                                  var td=document.getElementById(`p_q_${id}`);
                                  td.innerHTML   =qty ;

                                  var td=document.getElementById(`p_v_${id}`);
                                  td.innerHTML   =val ;
                                  </script>
                             </tbody>
                          </table>
